// Functions for creating and manipulating tables in the remote database.
// These functions may not be necessary once the tables are correctly configured,
// but I decided not to delete them for clarity and potential future use.
...
async function addPostCreationDate(pool) {
  const client = await pool.connect();
  try {
    const addDateToPostsQuery = `ALTER TABLE submissions
        ADD COLUMN creation_date TIMESTAMP NOT NULL DEFAULT NOW()
        `;
    const result = client.query(addDateToPostsQuery);
...
async function addPostsDescription(pool) {
  const client = await pool.connect();
  try {
    const addDescriptionToPostsQuery = `
        ALTER TABLE submissions
        ADD COLUMN description VARCHAR(200) NOT NULL DEFAULT 'No description provided'`;
    const result = await client.query(addDescriptionToPostsQuery);
